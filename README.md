...
**Project:** Pelagic Synthesis Working Group - NES-LTER  
...
This repository is part of the **LTER Cross-Site Pelagic Synthesis Working Group** (*Interannual variability and long-term change in pelagic community structure across a latitudinal gradient*). The working group brings together four marine LTER sites spanning a wide latitudinal gradient to compare how pelagic communities respond to environmental variability at annual and longer time scales:

| Site | Description |
|------|-------------|
| **NES** - Northeast U.S. Shelf | Rapidly warming northwest Atlantic; focus on planktonic food webs and higher trophic levels |
| **CCE** - California Current Ecosystem | Coastal upwelling biome with 70+ year CalCOFI time series |
| **NGA** - Northern Gulf of Alaska | Subarctic, high freshwater input, large spring bloom followed by smaller summer community |
| **PAL** - Palmer, Antarctica | One of the fastest-warming regions on Earth; sea ice dynamics central to food web structure |

The working group is pursuing three interconnected projects:

1. **Zooplankton size spectra / NBSS** = *this repository*
2. **Trophic Amplification**:  testing model predictions that climate-driven biomass declines are amplified at higher trophic levels and lower latitudes
3. **Double Integration Hypothesis**:  examining how atmospheric forcing propagates through pelagic populations
...
| Dataset | Source |
|---|---|
| Zooplankton abundance (335 um bongo net) | [EDI: knb-lter-nes.25.2](https://portal.edirepository.org/nis/mapbrowse?packageid=knb-lter-nes.25.2) |
...
### ESD calculation
Each organism is approximated as a sphere. ESD is calculated from carbon weight via:

```
C -> DW (/ %C/DW)  ->  WW (/ 0.20)  ->  volume (/ 1.05 g/cm3)  ->  ESD = (6V/pi)^(1/3)
```

### NBSS
Organisms are binned into log_2 (octave) ESD bins. Biomass per bin is normalized by bin width (mg C m-2 um-1). The NBSS slope is estimated by fitting a log10-log10 linear regression per station:
